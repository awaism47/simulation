...
class ManufacturingSimulation:
    ...
    def setup(self):
        self.resources = {step['name']: simpy.Resource(self.env, capacity=step['resource']) for step in self.process_steps}
        for step_name, resource in self.resources.items():
            print(f"Resource '{step_name}' has a capacity of {resource.capacity}")
